...
generate_null_sig <- function(names, signature) {
        ...
        sig_seed <- substitute(sig_name) %>%
                ...
                as.numeric() %>%
                sum()
        sample(rownames(skcm), size = length(signature))
}
...
skcm <- skcm[(rownames(skcm) != "" & !is.na(rownames(skcm))), ]
...
write_rds(signatures, "./data/tcga-skcm/C_01_signatures-with-null.Rds")
