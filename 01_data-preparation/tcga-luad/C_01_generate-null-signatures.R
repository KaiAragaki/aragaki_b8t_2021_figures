...
generate_null_sig <- function(names, signature) {
        ...
        sig_seed <- substitute(sig_name) %>%
                ...
                as.numeric() %>%
                sum()
        sample(rownames(luad), size = length(signature))
}
...
luad <- luad[(rownames(luad) != "" & !is.na(rownames(luad))), ]
...
write_rds(signatures, "./data/tcga-luad/C_01_signatures-with-null.Rds")
